# python-app-2 — Technical Documentation
...
| **Framework** | Flask |
| **Container registry** | Docker Hub (`jtandy/python-app-2`) |
...
flowchart LR
    ...
    K8s -->|serve| App[python-app-2 Pod]
...
3. The CD job updates `charts/python-app-2/values.yaml` with the new image tag and commits the change.
4. Argo CD syncs the `python-app-2` application, rolling out the new image to the cluster.
...
```bash
docker build -f dockerfile -t python-app-2:local .
docker run -p 5000:5000 python-app-2:local
```
...
The primary deployment mechanism is the Helm chart at `charts/python-app-2/`.

| Setting | Default | Description |
|---|---|---|
| `replicaCount` | `1` | Number of pod replicas |
| `image.repository` | `jtandy/python-app-2` | Docker image repository |
...
| `ingress.hosts[0].host` | `python-app-2.local` | Ingress hostname |
...
```bash
helm upgrade python-app-2 ./charts/python-app-2 \
  --set image.tag=<commit-sha>
```
...
3. Builds and pushes `jtandy/python-app-2:<short-sha>`
...
2. Updates `charts/python-app-2/values.yaml` with the new image tag using `yq`
3. Commits the values change back to the repository
4. Installs the Argo CD CLI
5. Logs in to Argo CD and syncs the `python-app-2` application
...
- **GitHub slug:** `jtandy13/python-app-2`

Import this file into your Backstage catalog to surface the service in the developer portal.

## Operational Notes

- **Ingress host:** Add `python-app-2-.local` to your hosts file or DNS to reach the service locally during development.
